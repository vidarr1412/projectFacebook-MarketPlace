...
import { FiSun, FiMoon, FiBell, FiUser, FiSearch, FiPlusCircle, FiArchive, FiHelpCircle } from 'react-icons/fi'
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 text-black dark:text-white">
      <header className="h-14 bg-white dark:bg-gray-800 flex items-center justify-between px-4 sm:px-6 shadow-sm border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center gap-2">
          ...
          <a href="/" className="flex items-center gap-2">
            <div className="bg-blue-600 w-8 h-8 flex items-center justify-center text-white font-bold rounded-full ml-[300px]">F</div>
      ...
      <div className="flex flex-col sm:flex-row ml-[270px] mt-[20px]">
        <aside className={`bg-white dark:bg-gray-800 w-full sm:w-[260px] sm:ml-[50px] p-4 space-y-6 border-r border-gray-200 dark:border-gray-700 h-auto sm:h-[calc(100vh-3.5rem)] overflow-y-auto z-20 sm:fixed transform transition-transform duration-300 ease-in-out ${menuOpen ? 'translate-x-0' : '-translate-x-full'} sm:translate-x-0`}>
          {/* Sidebar Content */}<div className="space-y-2">
          ...
          <div>
            ...
            <div className="space-y-3">
              ...
              <select value={priceFilter} onChange={(e) => setPriceFilter(e.target.value)} className="w-full p-2 border dark:bg-gray-700 dark:text-white rounded">
              ...
              <select value={dateFilter} onChange={(e) => setDateFilter(e.target.value)} className="w-full p-2 border dark:bg-gray-700 dark:text-white rounded">
            ...
            <div className="space-y-2 mt-6">
              ...
              <ul className="space-y-1">
                {categoryList.map((cat) => (
                  <li key={cat}>
                    <button className={`w-full text-left px-2 py-1 rounded hover:bg-blue-100 dark:hover:bg-blue-900 ${categoryFilter === cat ? 'bg-blue-200 dark:bg-blue-800 font-medium' : ''}`} onClick={() => setCategoryFilter(cat)}>
                      {cat}
                    </button>
                  </li>
                ))}
    ...
    <main className="flex-1 p-4 mt-6 sm:mt-[50px] sm:ml-[310px]">
          <h2 className="text-xl font-semibold mb-4 sm:mb-6">Today's picks</h2>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:gap-2 gap-3 mb-6">
            <div className="relative w-full sm:w-auto flex-1">
              <input
                ...
                className="w-290 px-4 py-2 pr-10 rounded border dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                onChange={(e) => setSearchQuery(e.target.value)}
              />
              <FiSearch className="absolute top-2.5 right-1 w-[25px] h-[25px] text-gray-500 dark:text-gray-300" />
            </div>
            <div className="w-full sm:w-auto">
              <Button className="bg-black text-white hover:bg-gray-800 w-full sm:w-auto mr-[300px]">Search</Button>
            </div>
          </div>
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mr-[300px]">
